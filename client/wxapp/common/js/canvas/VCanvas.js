...
VC.Image = function() {
    ...
    //圆角矩形遮罩
    function _updateRoundRect(ctx) {
        let radius = s.roundRadius;
        let roundRectOffsetX = s.roundRectOffsetX;
        let roundRectOffsetY = s.roundRectOffsetY;
        ctx.save();
        ctx.beginPath();
        ctx.arc(s.x + radius + roundRectOffsetX, s.y + radius + roundRectOffsetY, radius, Math.PI, Math.PI * 3 / 2);

        ctx.lineTo(s.width - radius + s.x - roundRectOffsetX, s.y + roundRectOffsetY);
        ctx.arc(s.width - radius + s.x - roundRectOffsetX, radius + s.y + roundRectOffsetY, radius, Math.PI * 3 / 2, Math.PI * 2);

        ctx.lineTo(s.width + s.x - roundRectOffsetX, s.height + s.y - radius - roundRectOffsetY);
        ctx.arc(s.width - radius + s.x - roundRectOffsetX, s.height - radius + s.y - roundRectOffsetY, radius, 0, Math.PI * 1 / 2);

        ctx.lineTo(radius + s.x + roundRectOffsetX, s.height + s.y - roundRectOffsetY);
        ctx.arc(radius + s.x + roundRectOffsetX, s.height - radius + s.y - roundRectOffsetY, radius, Math.PI * 1 / 2, Math.PI);
    ...
    //圆角梯形遮罩
    function _updateTrapeRect(ctx) {
        let radius = s.roundRadius;
        let roundRectOffsetX = s.roundRectOffsetX;
        let roundRectOffsetY = s.roundRectOffsetY;
        let startX = s.trapeOffset;

        ctx.save();
        ctx.beginPath();
        ctx.lineTo(s.x + startX, s.y);
        // ctx.arc(s.x + startX + radius, s.y + radius, radius, Math.PI, Math.PI * 3 / 2);
        ctx.lineTo(s.x + s.width, s.y);
        // ctx.arc(s.width - radius + s.x, radius + s.y, radius, Math.PI * 3 / 2, Math.PI * 2);     
        ctx.lineTo(s.x + s.width - startX, s.height + s.y);
        // ctx.arc(s.w/idth - radius + s.x - startX, s.height - radius + s.y, radius, 0, Math.PI * 1 / 2); 
        ctx.lineTo(s.x, s.height + s.y);
        // ctx.arc(radius + s.x, s.height - radius + s.y, radius, Math.PI * 1 / 2, Math.PI);
        ctx.lineTo(s.x + startX, s.y);
        ctx.clip();
        ctx.drawImage(s.source, s.x, s.y, s.width, s.height);
        ctx.restore();
        ...
        if (s.source) {
            ...
            switch (s.fillType) {
                case "circle":
                    ...
                    break;
                case "roundRect":
                    _updateRoundRect(ctx);
                    break;
                    // case "trapeRect":
                    //     _updateTrapeRect(ctx);
                    //     break;
                default:
                    _updateNormal(ctx);
                    break;
    ...
    s.addChildByList = function(list) {
        ...
        if (len > 0) {
            ...
            function addobj(list) {
                ...
                switch (obj.type) {
                    case "image":
                        if (obj.path.indexOf('http') != -1 || obj.path.indexOf('https') != -1) {
                            wx.getImageInfo({
                                src: obj.path,
...
VC._C_IMG = function(obj) {
    if (obj && obj.path) {
        ...
        //图片选择中心X位置
        img.anchorX = obj.anchorX || 0;
        //图片选择中心Y位置
        img.anchorY = obj.anchorY || 0;
        //图片缩放X
        img.scaleX = obj.scaleX || 1;
        //图片缩放Y
        img.scaleY = obj.scaleY || 1;
        //绘制类型 目前只有默认 和圆形两种
        ...
        //如果是圆角矩形 需要传入圆角的弧度
        img.roundRadius = obj.roundRadius || 0;
        img.roundRectOffsetX = obj.roundRectOffsetX || 0;
        img.roundRectOffsetY = obj.roundRectOffsetY || 0;
        img.trapeOffset = obj.trapeOffset || 0;
